---
description: >-
  Scaffold a project-local /test-app skill into the CURRENT project: detect its surface — a web
  app, a full-screen terminal UI, or a command-line tool — then its stack (build tool, how it runs
  and is driven, base URL or launch command, in-repo helpers, auth model, credentials, security
  signals), and write a tailored `.claude/skills/test-app/SKILL.md` plus
  ...
  files. This GENERATES the test skill; it does NOT run it. NOT for: "test the app" / "verify the
  changes" right now — that is the generated `test-app` skill, not this one.
---

# Create Test App

Generates a `/test-app` skill tailored to the current project. The generated skill verifies that recent changes to the running app work: a web app is driven through HTTP and a browser, a terminal app through a real terminal and its own binary. Either way it reports concrete pass/fail.

The two-file shape and your job are the same for every project: detect the details and fill them into the templates. What changes is **which** pair of templates — one decision, made once, at the start.
...
Everything the generated skill owns lives **under its own directory** (`.claude/skills/test-app/`) — the credentials file and the generated `e2e/*.py` scripts — so generated artifacts never collide with the project's real `scripts/` (e.g. an existing `scripts/api.py`) or a repo-root creds file. The generated skill references them by the project-root-relative path `.claude/skills/test-app/…`.
...
If that fails (not a git repo), use the current working directory as the root and note that the generated skill's no-argument git-diff scoping will be limited — it leans on conversation history instead. Everything below is relative to this root.
...
table below. It is also the one detection whose failure is invisible: a wrong base URL produces a
skill that obviously does not work; a wrong surface produces a plausible skill that asks the wrong
questions of the right program.

For `tui` or `cli`, read `references/process-surfaces.md` for the rest — a separate file so the
common case does not pay to read it.

The placeholder map is three tables, not one with a surface column: a terminal project has no
base URL to guess at.
...
**On the process pair, the surface itself is that value**, decided by running the program rather than reading it — `references/detection-guide.md` Step 0 B2. A dependency is not a surface: `ratatui` sits in `[dev-dependencies]`, `rich` prints coloured tables from a plain CLI. When the static evidence is not unanimous and the runtime probe cannot answer, ask.

**If the repo has more than one compose file** (`docker-compose.yml` + a `.dev.yml`/`.prod.yml`/`.override.yml`), they are different deployments with different URLs and `compose -f` commands — the skill can't know which is the test target. List the candidates and **ask the user which deployment `/test-app` should run against**, then bake the choice into the URL/run-model and the compose commands.

### 3. Summarize what you found

Show the user a compact table. **Surface is the first row**, with how it was decided — read
statically, settled by the runtime probe, or asked — because everything else hangs off it and its
error is invisible in the result.

Then, for `web`: stack, run model + base URL, HTTP tool (helper vs curl), UI in scope?, auth
model, creds-file status, extra services, and which conditional catalog items activate (IDOR /
...
available, and which conditional items activate (mouse / colour / stdin / signals / the three
security flags).
...
and use the answers. Two asks are **mandatory** — the same rule twice: several plausible targets,
no way to rank them from the code, and a wrong pick that stays invisible until someone reads the
generated skill.
...
- **Both a web signal and a terminal entrypoint exist** — which surface `/test-app` should target.
  ...
  server, and only the user knows.
...
If `.claude/skills/test-app/` exists (a hand-tuned skill), **do not overwrite silently**: show what would change (a short diff of intent), and overwrite only on the user's explicit confirmation. If they decline, stop.

**The marker upgrade case:** a skill with no `<!-- test-app-surface: … -->` marker. Without it, `/r:task-review` infers the surface from whether a base URL appears in the file — right only for web skills, wrong for terminal ones. Re-running fixes it in one line; while there, re-detect: a project that grew a CLI since the skill was written has a second surface nobody asked about.

**The worktree upgrade case:** a compose-based skill with no *Where the app runs* section. Its **manual** `/test-app` runs hit the fixed default port and collide across worktrees. (The automated verifier path is safe regardless — `/r:task-review` Step 8a deploys through the shared helper whatever the skill contains.) Offer the upgrade: re-detect the compose knobs (step 2) and rewrite the skill with the `{{#IF_COMPOSE}}` section added, leaving the rest of the user's tailoring intact.
...
the web ones — absent, "a skill without one" and "a web skill" are indistinguishable, and the
pipeline falls back to inferring from whether a base URL appears in the file.

### 7. Write the creds stub if needed

If `.claude/skills/test-app/test_creds.txt` is absent, write it from `assets/test_creds.txt.template`, add `.claude/skills/test-app/test_creds.txt` to `.gitignore` (it holds secrets — ask first), and tell the user to fill in real accounts. Never copy real secrets into the skill files — they only reference the creds path. If the project already keeps credentials elsewhere (e.g. a repo-root `test_creds.txt` or `.env`), note it in the summary but still standardize on the skill-dir creds file; seed it from the existing one with the user's OK.

### 8. Report

List the files created, name the surface and how it was resolved, summarize the tailoring in a sentence or two, and show how to use the result:
...
Suggest a first run. On a `tui` project where tmux was missing, say so in the same breath: the skill is written and correct, and the terminal checks stay unrun until tmux is installed.
...
to asking is a detection gap with a name.

**`surface` is `web` | `tui` | `cli`, and `surfaceDetectedBy` is `static` | `probe` | `asked`.**
The second is the same detected-versus-asked reading at the point where a wrong answer costs the
most, since the surface picks the template pair. A framework that always ends up `asked` is a
discriminator gap with a name. `surfaceCandidates` of 2 or more means the hybrid ask fired, which
shows whether that ask is over- or under-triggering. `tmuxAvailable` makes "the terminal checks
did not run" countable rather than inferred, so a run of them on one machine is visible instead of
a quiet pass.

**`credentialsFound: false` is a normal outcome, never a blockage.** The generated skill works
without test credentials; it cannot log in. Recording it as a block would put a scaffold that
...
- **A library with no executable entrypoint** — no surface at all. **Stop and say so** rather than emitting a web skeleton full of `TODO`s: a `/test-app` that wraps `mvn test` or `cargo test` is a rename, not a skill. Tell the user what you looked for and what would change the answer (a `[[bin]]`, a `cmd/*/main.go`, a console script).
- **tmux is absent on a `tui` project** — write the skill anyway, and say plainly what it will and will not do: the terminal checks are recorded as not run and named, the rest still run. Do not fall back to the web pair or to `cli`; the surface is a fact about the app, not about this machine.
- **Worktrees need the skill on their branch.** The generated skill is loaded at session start from the checkout's own `.claude/skills/`. Track it in git so worktrees carry it — but a worktree branched off a commit from *before* the skill landed won't have it, so `/test-app` silently won't load there. Tell the user to commit the skill and to base/rebase worktrees on a commit that includes it. (`r:task-review`'s Step 8 detects this stale-worktree case and says so rather than reporting "no skill configured.")
