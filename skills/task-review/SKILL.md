...
**Spend depth where judgement happens.** This skill's frontmatter sets `effort: high`, which every subagent would otherwise inherit — including ones whose whole job is running a shell command. The workflow tiers the fan-out instead. The test is not "does this step matter" (they all do) but **"does this agent decide anything?"** — a wrapper around a tool that decides for it does not, and neither does a fixer whose finding was already judged real by someone else. So the pre-warm, teardown, stats sink and issue-filer run at `low`; the build runners, UI deploy, Phase 0 triage and both Codex tracks at `medium` (Codex does the reviewing, the agent shells out and parses); the docs hunter at `medium` too (a bounded match against a reference file, and doc drift is never auto-fixed). The three fixers — `fix-correctness`, `end-verify-fix`, `ui-fix-minor` — are the exception to "pinned": their provider, model and effort come from `steps.fix` in the config, resolved by the workflow itself, and on `provider: codex` they drive the Codex CLI while the tier shown here is the Claude subagent driving it. The rule the setting is held to is the same one a pin encoded: never deeper than the implementers whose code they patch. The readability refactor is not one of them — it invokes `/r:code-refactor`, and its judgement is re-formed by nothing downstream. What runs at the inherited depth is the set that forms an opinion nothing downstream re-forms: `r:code-quality`, the fix-triage that decides what is a false positive, `/r:code-scan`'s triage of its own findings, and the readability refactor. The three pattern hunters and the two UI halves sit there too, but **pinned** rather than inherited — see the next paragraph for why the pin is load-bearing even where the number matches.
...
**And spend fewer turns where nothing is being judged.** Depth is one axis; the other is how much a subagent reads before it starts. The hunters are the review's largest line item, and across ~380 measured runs the cost is not the reasoning — the median hunt takes ~49 turns and grows to ~93k tokens of context, most of it whole source files opened *before the diff is*. Three things hold that down without retiring a track or narrowing what gets reviewed. All three pattern hunters — security included — run on `r:bug-hunter-pattern`, a four-tool sweep agent, rather than the single-bug investigator whose reproduce-first persona pulls a sweep into a deep read. Every hunter's brief orders the hunt — change first, judge from the hunk, open source only for a candidate it can't settle — and gives it a ~12-call budget it must *report* overrunning or falling short of. And the diff is captured once, after triage, so N hunters read one file instead of each deriving its own, which also stops them reviewing three different changesets. The docs hunter is additionally pinned to `sonnet`: it matches code against written statements and its findings are never auto-fixed, so it is the one track where a cheaper model costs a user a read rather than a wrong edit. `references/prose-pipeline.md` carries the numbers.

**Model tiers run the other way too, where an agent is cheap by default but one of its calls is not.** The build-runner agents (`r:maven-build-runner`, `r:gradle-build-runner`) are `haiku`, which fits what they do on nearly every dispatch: run one command and report `BUILD SUCCESSFUL`. The *classifying* build call steps back up to `sonnet`, because on a red build it has to split failures into in-scope and pre-existing — and that split is load-bearing both ways: wrongly "pre-existing" halts the run on a failure the fix phase should have taken, wrongly "in-scope" sends a fixer to edit somebody else's failing test. The post-scan rebuild has nothing to classify either — the tree was green before it, so any failure is in-scope by construction — but it stays on `sonnet` too, because it still decides green vs red, and there that verdict halts the run outright with no tier above it to disagree. Which is also why every build prompt names the deciding rule rather than assuming it: **judge from the exit code, never from the log text.** The incremental commands are `-q`, so a green build prints no `BUILD SUCCESS` line to find and does print `[ERROR]` lines — failure-path tests, and Surefire's `going to kill self fork JVM` shutdown notice — and an agent grepping for the first while seeing the second calls a finished, green diff red.

## Non-negotiables

The full text of each lives in `references/prose-pipeline.md` (between Steps 8 and 9). These govern **both** engines:

- **This routine never fires on its own — but it is reachable.** It runs on an explicit `/r:task-review`, or when `/r:task-run` reaches its post-task-review step and invokes it through the Skill tool. It does **not** run because a coding turn ended, a build went green, or a diff looked reviewable. The rule is held **here and in the description, never in frontmatter**. `disable-model-invocation: true` is the wrong instrument: it doesn't distinguish "the model auto-loaded this" from "the model was told to run this", so it also blocks `/r:task-run`'s mandatory Step 5 and the pipeline's own caller cannot reach it. Don't add it.
