...
Record the answer as **hasTestApp**. It is a disk check, and a gitignored, locally-scaffolded `SKILL.md` counts — git has no opinion here. **Run the command; do not answer from the directory listing.** The `.claude/skills/test-app/` directory outlives its `SKILL.md` (screenshots, `e2e/`, `bugs/` all accumulate there and are gitignored too), so a glance says "present" over a tree that has no skill in it — measured 2026-08-19, one triage of four did exactly that, and the run spent a docker pre-warm, an 86s deploy and both UI halves before they discovered the file was gone. Step 8's deploy re-checks before it spends anything, and an absent file there is a **skip**, not a blocked track. See Step 8.
