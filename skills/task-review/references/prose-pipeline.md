...
**Check the scope the skill actually read.** `/security-review` resolves its own diff scope and ignores the one it is handed on about one dispatch in seven (7 of 46 recorded) — it reads the unpushed branch commits when it was given the working tree, or the reverse. So compare what its report says it covered against what you asked for. On a mismatch the track is **not** a clean bill: the report is real, complete, and about a changeset this review is not certifying. Say so, name both scopes, and treat the security half exactly as you treat a blocked one — the scan did not cover this diff. Give it its own wording, though: a blocked tool has to be made to run, a drifted one has to be made to read the right thing.
