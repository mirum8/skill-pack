...
The `docs` row is dispatched with the same brief but is **not** part of this step's join — see the note above Step 2a. Nothing waits on it until the fix-list is assembled.
...
Same for the docs hunter's *other* input: discovering the doc tree with `ls`/`find` costs it 25 shell calls and 141k characters of tool output per dispatch, the most shell-heavy hunter of the four. You already walked this repo in Step 0 — hand it the doc list you found, and tell it to Glob only for a file missing from that list.
