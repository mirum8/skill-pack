...
- **The post-task review routine lives in the `/r:task-review` skill, not in `CLAUDE.md`, and it is not run automatically.** It fires only inside `/r:task-run` (which invokes it as a mandatory step) or when the user explicitly asks. `r:claudemd-patch` installs nothing for it and removes any leftover pointer/checklist that would make it auto-run in regular sessions. The routine itself (real reviewers `/r:code-bugs` + `r:code-adversarial` → fix → build → `/r:code-scan` → rebuild, Maven/Gradle auto-detected, no faking, STOP on missing prerequisites) is documented and maintained in `${CLAUDE_PLUGIN_ROOT}/skills/task-review/SKILL.md`.
