...
`/r:plan-run` executes a plan file. `/r:spec-design` writes the canonical shape, but a hand-written
plan is executable too; the difference is how much a phase block states versus how much has to be
classified. This file is the whole of that contract.
...
- **A phase is done when every one of its `- [ ]` items is ticked.** A partly-ticked phase goes back
  in the run list — the implement Workflow reads the whole block, so a resumed phase re-reads
  criteria the code already satisfies.
- A ticked phase **heading** (`### Phase 4 — … ✅`, `- [x]`-style markers, `~~struck through~~`) is
  respected too, but the items decide. A heading marked done over unticked items is a disagreement
  — trust the items and say so in the report.
...
criteria are left empty for the planner to derive, so everything the plan author wrote down is lost.

`<path> / <something>` is also how a *list item* is handed over; `Phase <n>` is what makes the
workflow read the block rather than one checklist line, so keep the literal word `Phase`.
...
After the review and the `Done when:` check, with the branch confirmed, before staging:

- Flip **only the items that were implemented and verified**, `- [ ]` → `- [x]`. A phase that
  half-landed leaves half its boxes ticked — never tick an item to make a phase look finished.
- When every item is ticked, mark the heading too, and append the branch:
...
- **Replace the `Files:` line with what the phase actually touched**, from the branch rather than
  ...
  `Files:` is written before any of the code exists, so it names the files a feature *carries* and
  never the ones it must touch to be wired in, which are unknowable until there is an app to wire
  into. The gap is not small: one phase declared three files and changed eleven, and the eight it
  did not declare were the hub files every other phase in that package also reaches.
  ...
  wave that builds for hours and then will not merge.
...
- **Idempotent**: an item already ticked, or a heading already carrying the marker, is left exactly
  as it is. A phase that has just built is the only thing in the pipeline that knows the true
  answer, and this is the only moment it is still holding it. The `Files:` line is the one
  exception: it is rewritten from the diff every time the
...
- Re-locate every item by its **verbatim text**, never by a line number read at parse time — the
  number stops being true the moment anything edits the file. If the text is gone, **do not guess**:
  leave the file alone and report the phase as built-but-unticked, naming it.
- Never restructure the document: no reflowing, no moving a phase under a different heading, no
  renumbering, no tidying a hand-written checklist.
- The ticks are staged into the phase's single commit, so "built" and "done" revert together. When
  ...
  place and say so in the report, since reverting the phase then leaves the plan claiming the work.
- `--dry-run` writes nothing, ever. A halted phase writes nothing either.

## A hand-written plan

A plan that is only headings and checkboxes still runs. What it loses is specific, and is named to
the user once, at the gate, rather than discovered per phase:
...
- **No `Done when:`** → the review is the only gate. It certifies the diff is sound; nothing checks
  ...
  which certainty the run bought.
- **No `Files:`** → the per-phase re-check has less to compare against and answers "already done?" on
  weaker evidence. Still worth running; believe an `already-done` verdict a little less.
- **Numbering gaps or repeats** → a hard stop, because `Phase <n>` is the handoff string and an
  ambiguous `n` sends the implementer at the wrong block — one of the two `check_todo.py` findings
  this skill refuses to run through.
