...
description: >-
  ...
  test-first, have the REAL Codex reviewer read the finished diff, verify every finding against
  the actual code, and fix only the ones that hold and belong to this change. No planning phase,
  no subagents, no Workflow, no feature branch, no PR, no commit, and none of `/r:task-run`'s
  fan-out (no explorers, plan review, bug hunters, `/r:code-scan` or UI pass). Use whenever the
  user says "/r:task-quick", "quick fix: …", "just fix this and review it", "small change, skip
  the whole pipeline", or hands over a one-or-two-file change they want built green and read by a
  real reviewer rather than shipped through the long routine — even if they don't name the skill.
  It opens with a scope check: a task that needs a design decision, touches auth, money,
  persistence, concurrency or security, needs a migration, or spans more than about five files
  goes back to `/r:task-run` before any code is written. NOT for: work you want planned and
  reviewed in depth or shipped behind a PR (`/r:task-run`), reviewing a diff that already exists
  (`/r:task-review`), a whole backlog (`/r:issues-fix`), or a phased plan (`/r:plan-run`).
effort: high
...
size, writing the plan and writing the code are the same act. What stays is the review: the
**real Codex reviewer** reading the real diff, never your own re-reading of code you just wrote.
...
Do every step yourself, here. No `Workflow`, no subagents, no background dispatch. There is no
fan-out to lose, so spawning would buy nothing and cost a round trip and a context re-read per
step; running inline also keeps the whole run visible to the user, which is what they chose over
`/r:task-run`. Everything you read stays in your context, so keep the reading targeted — grep for
the symbol, read the file that owns it, read its test.

Because it spawns nothing, this skill runs correctly anywhere — including inside a subagent, where
`/r:task-run` and `/r:task-review` cannot run at all: subagents have no `Agent` tool and no
`Workflow` tool, so their fan-out would silently collapse to one context and still report success.
Every step here is Bash, edits and your own reading, so nothing collapses.
...
Do not reword the task and continue. This is a check, not a planning phase: a couple of greps and
the reading Step 1 needs anyway.
...
`build.gradle[.kts]` → Gradle, neither → none). Work on the branch the repo is already on; if that
is `main`/`master`, say so once — the user may want to branch first — and carry on if they don't
object.
...
1. **Write the tests first** and **run them before touching production code.** Record what you
   ...
   Red-before-green is observed, never assumed. A test you expected to fail that passes on
   unmodified code is a signal: usually it is too weak to reach the bug (strengthen it),
   occasionally the behaviour already exists (say so, and treat it as a guard).
...
3. **Build with the tests**: `mvn -q test` or `./gradlew test`. Tests certify a change of this
   size; the clean packaging build `/r:task-run` runs is most of what makes it slow. A project
   with neither build file gets its own test command if it has one — and if nothing runs, say so
   rather than implying a build that never happened.
...
the diff is not a substitute — you wrote the code, so your opinion carries no independent
information. Say plainly that the change is unreviewed, tell the user how to install the plugin
(`/plugin install codex@openai-codex`), and finish the run.
...
Codex is a strong reviewer and is sometimes wrong, so **check each finding against the code**: open
the file and lines it cites and work out whether the failure it describes can happen. Give it one
verdict with a one-line reason you would defend to the user:
...
Rubber-stamping the whole list turns a quick fix into an unplanned refactor of somebody else's
code; dismissing whatever is inconvenient turns the review into theatre. The test for a dismissal
is whether the reason survives being read out loud — "Codex misread the null check, `orderId` is
validated at line 41" survives; "seems minor" does not.

Two cases resolve to neither and are **deferred**: a confirmed defect whose fix lands outside this
change's surface, and one whose fix needs a design decision. Report those with the finding intact
rather than fixing them — a quick fix that quietly rewrites a neighbouring module is exactly what
...
else. Do not re-run Codex afterwards: this pipeline reviews once; the next reader is the user.

If anything was fixed and there is a build tool, **run the build once more**. That is a
verification pass, not a second repair loop: if it goes red, report it plainly and leave the diff
for the user.
...
4. **The review** — that Codex ran (or did not), how many findings it raised, what you fixed, what
   ...
   user's to overrule, which they cannot do unseen.
...
`verdict` is your Step 3 judgement rather than Codex's claim: a reviewer whose findings never
survive verification scores the same as one that finds nothing until the dismissals are recorded
too. `fixed` is true only for findings you actually fixed, so a deferred one is `confirmed` with
`fixed: false`. Findings live under their own `quick-codex` track — same tool as `/r:task-review`'s
`codex` track, but a different mode and a much smaller change under it; merged, neither would be
readable. A review that never ran records `reviewRan: false` and **no findings at all**: a blockage
is not a finding, and writing one in makes a tool that could not run look like one that found
something.
