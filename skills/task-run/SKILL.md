...
**The one trap that survives** is the detached shell job. The Codex plan review runs foreground — which is what passing the companion no mode flag does, since `task` has no `--wait` and would fold one into the prompt — but a long review exceeds the ~600s Bash cap and moves to a **background Codex process — not an Agent child, so it never sends a completion notification, ever.** The workflow's Codex agent owns collecting it: **wait on the worker pid in one blocking Bash call** — a shell loop bounded below the cap, re-entered while the pid is still alive — and then read the finished review from the job record's `rendered` field. One blocking wait, never a check per turn: each turn re-reads the whole context, and each check is another moment at which the agent can decide a live run looks stuck. Never wait on output-size stability — the log goes quiet for minutes mid-reasoning, so "it stopped growing" is a false done. Nor is the record's `status` a liveness check: a worker that is killed or crashes never writes a terminal status, so a dead pid over a record with no `rendered` is a job that DIED, and polling `status` there waits out the whole cap on a field that will never change. A "came to rest / no live children" signal proves no child *agent* is running; it says nothing about a detached shell job.
