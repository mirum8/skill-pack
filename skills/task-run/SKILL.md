...
- **Task source** is one of four things, detected in this order:
  ...
  4. **Free-text description** — anything else that describes concrete work (e.g. `add a dark-mode toggle to the settings page`). This is the fallback: if the argument isn't an issue ref, a todo file+phase, or a file+item locator, treat the whole argument as the task description.
...
in `<project>/.config/skill-pack.yaml`, falling back to the pack's `.config/defaults.yaml`. The
workflow resolves that itself and logs which file it read and every value it had to substitute, so
a run that implemented on a different provider than you expected says so rather than leaving you to
infer it. Point the user at the project file if they want it changed; nothing here overrides it.
...
| Plan | brief | full Fable plan | full Fable plan |
...
**The design phase is gated on `uiVisualChange`, which is a different question.** `uiTouched` asks whether a frontend *file* changes; that is the right question for the UI verifier, since a change meant to render identically is precisely one worth looking at. The design phase can only produce a *decision about what the user sees*, so it asks whether anything renders differently. Self-hosting a font, a CSP header, a class rename, a build-pipeline change — all edit `.html`/`.css` and decide nothing, and an acceptance criterion reading *"the appearance does not change"* is the giveaway. One exception, and it is the case the phase was built for: when the **explorers** turned `uiTouched` on, the description hid the frontend entirely, so Phase 0 never judged the visuals at all — that counts as unanswered and the design phase runs.

**Unclear calls land on standard, not full.** That is the point of having it: standard still gets a real Codex read of the diff, the security hunter, doc-drift checking, static analysis and a green build, so `full` can mean the one narrow thing it should — *this approach needs challenging before code is written*. What standard trades away is the other `/r:code-bugs` pattern hunters (their performance-at-scale lens has no dedicated reader below full), the up-front adversarial pass and `/r:code-quality`. A tier that absorbs every uncertainty ends up meaning nothing, which is exactly what happened when the only options were light and full.
...
**Write the review back into the plan — before Step 6 commits it.** Post-task-review returns `appliedFindings`: one entry per real finding it acted on, `{ track, verdict, fixed, description }` — the correctness and readability fixes it applied, the doc-drift it surfaced, and any end-verify item left unresolved (dismissed findings are already filtered out). Append them to the plan at `handoff.planPath` as a `## Post-review changes` section, one bullet each — `- [<track>] <description> (fixed)` when `fixed` is true, `(unresolved)` otherwise — mirroring the "Plan review changes" note plan-review already leaves for the pre-implementation pass. **In the same edit, set the plan's `status:` header to `done`.** The plan is tracked now and lands in Step 6's single commit, and the reuse-index links back to it — so this is what keeps the committed, index-linked plan honest about what was *built*, not only what was *planned*. If `appliedFindings` is empty, omit the section but still flip the status to `done`. This replaces the old status-flip that lived in Step 6: the plan must be final before it is staged.
...
2. **One commit — the only commit of the run.** Now, and only now, stage everything on the feature branch — the implementation, the review/fix changes from Step 5, the plan file at `.task-plans/<slug>.md` (tracked now, carrying `status: done` and its `## Post-review changes` section), and (todo source only) the checkbox ticks — and make a **single commit**. Nothing was committed before this point, so this lands the whole task as one commit, after the review. (If, on a resume, an earlier interrupted run already created a commit, fold the remaining working-tree changes in with a normal commit rather than forcing history rewrites — honest history beats a cosmetically-perfect single commit.)
...
The plan's `status: done` and its `## Post-review changes` section were written at the end of Step 5, so by the time this commit is staged the plan is already final — there is nothing left to flip here.
...
- **`.task-plans/` is tracked, not gitignored.** The plan is committed with the task at finish (Step 6), carrying `status: done` and its `## Post-review changes` section, and the reuse-index links back to it — a gitignored plan would be a dead link for everyone but the author. The scribe must not add `.task-plans/` to `.gitignore`, and must remove that line if a prior run added it. (Migration: a repo that already ignores `.task-plans/` keeps its old plans untracked until that line is dropped.)
