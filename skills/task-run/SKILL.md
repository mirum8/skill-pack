...
**Skip this step when `handoff.resume.reviewDone` is true** — a resumed run whose ledger recorded a passed review over exactly the tree that is there now, with nothing re-dispatched since. The plan already carries `status: done` and its `## Post-review changes`, so go straight to Step 6 and say in the report that the review was carried over from an earlier run. On anything weaker — `reviewDone` false or absent — the review runs in full.
...
**Then record the passed review in the plan's ledger**, as the last thing in Step 5 — `python3 "${CLAUDE_SKILL_DIR}/scripts/plan-ledger.py" mark --plan <handoff.planPath> --base <handoff.base> --key review`. Only after a review that passed: the line tells a later run that this exact tree was reviewed, and a run stopped between here and the commit then resumes straight to Step 6 instead of reviewing the same diff again. A mark that fails costs that shortcut and nothing else — say so and continue.
...
- **Resume is single-agent, in place — re-invoke `/r:task-run` on the same task.** The partial work lives on the feature branch's working tree, *uncommitted* until Step 6, and the plan persists at `.task-plans/<slug>.md`. Its header is a **ledger** the implement workflow reads back through `scripts/plan-ledger.py`: `status:` (`reviewing → implementing → done`), a `reviewed:` stamp written after a real Codex plan review, one `slice <label>: done` line per implementer that returned clean, a `build: green` line and a `review: done` line. The last three carry hashes of what they covered, so a resume skips a step only when the tree still holds exactly what that step produced:
