---
description: >-
  Modern Java/Kotlin test practices (Given/When/Then, AssertJ, JUnit 5, self-contained tests,
  hard-coded expectations, helper functions, KISS > DRY). Consult it automatically and proactively
  — no explicit request needed — for ANY task that produces or changes Java/Kotlin behavior worth
  testing, not just tasks that name "tests": writing, generating or modifying test code of any
  kind (unit, integration, bug-reproduction, refactor, added coverage); implementation that should
  come with tests — adding or changing a service method, controller/endpoint, repository,
  validation, mapper, or business logic; reproducing or fixing a reported bug (write the failing
  test first); refactoring production code that needs a safety-net test. Before writing or editing
  a Java/Kotlin .java/.kt file that carries logic, or when asked to implement, build, add, fix, or
  change such code, load this skill FIRST and let it shape the tests. Bias strongly toward
  triggering. NOT for: pure read/explain questions, build/dependency/config-only edits, or non-JVM
  languages.
---
...
Writing tests is context-heavy work — exploring existing patterns, reading the code under test, drafting many test methods, running them, reading failures — and doing it inline crowds out the larger task the tests belong to. When you can, **hand the writing to a dedicated `r:java-backend-developer` subagent** and keep the orchestrating context lean.

**First — can you delegate?** You can only spawn a subagent if you have an Agent/Task subagent tool, and a subagent cannot spawn another subagent. If you are yourself a subagent (including one spawned to write these tests), you have no such tool — don't search for one. Skip straight to the Workflow below and write the tests inline. This is also what prevents delegating in a loop.
...
2. Spawn the subagent(s):
   ...
   - **Large target (many files/classes):** fan out — several `r:java-backend-developer` subagents in parallel, each owning one slice (e.g. one service plus its tests), with a shared one-line style note ("match the existing test conventions in `<module>`") so the slices stay consistent.
...
For a single trivial test, write it inline — delegating would cost more than the work.
...
Integration tests are the default; unit tests are reserved for methods with complex algorithms, many branches, or intricate logic. Prefer complete vertical slices (HTTP → business logic → database) over isolating each class with mocks — that tests behavior, not implementation, and survives refactoring.

## Don't Mock the Integration

An integration test exists to verify that real components work together. Spin up a full Spring context and a real database but `@MockitoBean` every interesting collaborator (the AI responder, the embedding service, the outbound sender), and you've built a unit test wearing an integration-test costume: it passes when the orchestration is broken.
...
Now you can assert on the table state after the action runs, not only on whether the mock was called.
...
Use `actual*` and `expected*` prefixes for variables in equality assertions — it prevents mix-ups:
...
Extract repetitive setup and assertion details into descriptively-named helpers — the primary mechanism for keeping tests concise:
...
Don't extract values used once or twice into variables. Inline values are shorter and easier to trace in failure messages:
...
Never extend an existing test to cover additional cases. Create separate, descriptively-named test methods; each documents one behavior and makes failures easy to diagnose:
...
Helpers must expose the values relevant to the test, so readers never jump to function definitions:
...
Compare output against hard-coded expected values. Reused production mapping logic can't catch its own bugs:
...
AssertJ gives fluent, type-safe assertions with descriptive failure messages. Never use `assertTrue`/`assertFalse` — they produce cryptic output:
...
Repeating `assertThat(x)` per line is noise. Chain on the same subject — failure messages stay as clear, and the test reads as one statement about one thing:
...
The verify-only test — stub some inputs, call the method, `verify(mock).something()`, stop — passes even if the method becomes a no-op in production, because it never checks what the user (or the next layer) sees. It also breaks on every refactor, because it asserts *how* the code works, not *what* it does.

Assert on observable outcomes instead: return values, persisted rows, exceptions thrown, messages sent. Use `verify` only when the side effect is the behavior (e.g. notification dispatch) — and even then, capture the argument and assert on its contents.
...
When `verify` is needed, prefer `ArgumentCaptor` over `any()`. Capturing documents what the production code did, not only that it did *something*:
...
A test asserting "no work happened" must verify the *full* set of side effects didn't occur — not one. Otherwise a future bug that swaps which side effect fires will pass:
...
- **Pure delegation wrappers** — a one-line `delegate.x()` with no observable side effect has nothing meaningful to verify. Either delete the wrapper (often dead code or a port-fulfillment vestige) or, if it's load-bearing, write an integration test against the real delegate that proves the end-to-end behavior. A test asserting `verify(delegate).x()` only proves the wrapper compiles.

## Reflection in Tests Is a Smell

If a test reaches for `ReflectionTestUtils.setField(...)` or a custom reflection helper to populate fields, the entity is missing a test-friendly factory or builder. Fix the entity (a Lombok `@Builder`, `@Setter`, or a `withId(...)` helper) rather than working around it in tests. Reflection couples tests to internal field names; a rename will break them silently.
